...
def multiselect_with_select_all(label: str, options, key: str, default_all=True):
    ...
    cols = st.columns([3, 0.8, 0.8])
    with cols[0]:
        selected = st.multiselect(label, options=options, default=st.session_state[key], key=f"{key}_multi")
    with cols[1]:
    ...
    # Update session_state to reflect the multiselect choice
...
def plot_kpi_overtime(
    ...
    """Plots KPI over time (Branch vs Group level) with unique widget keys per tab."""
    ...
    view_level = st.radio(
        ...
        index=1,   # default to group-level
        horizontal=True,
    ...
    if view_level == "Branch-Level (many lines)":
        fig = px.line(
            ...
            x="CalendarDate",
            y=y_col,
            color="Group",        # Test vs Control colors
            line_group="Branch",  # One line per branch
            hover_data=["Branch","District","Golive","Value","Rolling7"],
            title=f"{kpi_label_choice} Over Time — Branch-Level ({view_choice})"
        )
    else:
        daily_grouped = (
            daily_filtered.groupby(["CalendarDate","Group"], as_index=False)[y_col].mean()
        )
        fig = px.line(
            daily_grouped,
            x="CalendarDate",
            y=y_col,
            color="Group",
            title=f"{kpi_label_choice} Over Time — Group Average ({view_choice})"
        )
    ...
    # 👇 give each chart a unique ID by tab + KPI
